	...
	methods: {
		...
		async selectConversation(conversation) {
			try {
				...
				// Set CONFERENCE property (RFC 7986) as the source of truth
				this.calendarObjectInstance.eventComponent.addConference(url, 'Nextcloud Talk', ['AUDIO', 'VIDEO', 'CHAT'])

				// Set Talk URL in location as display-fallback (only if location is empty)
				...
				showSuccess(this.$t('calendar', 'Successfully added Talk conversation link.'))
