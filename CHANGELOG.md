...
- **The reported user count included accounts that cannot log in.** Nextcloud counts a named user as an account with access to the environment — any backend, but it has to be able to log in — and invoices the end customer on that basis. Disabled accounts still exist and still own their files, but cannot log in, so they are not named users. The count now excludes them, taken in a single pass so the two figures cannot be read at different moments and disagree. The account total and the disabled count are still reported separately, so the difference between "accounts on this server" and "accounts being charged for" stays visible instead of implied. The settings page shows named users, which is also the figure the subscription suggestion tests against.
