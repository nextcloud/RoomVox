#!/usr/bin/env node
/**
 * Generate translationfiles/templates/roomvox.pot from l10n/en.json + PHP scan.
 *
 ...
 *
 * Output: translationfiles/templates/roomvox.pot (overwrites previous)
 */
...
let phpStrings = [];
try {
    const phpFilesList = '/tmp/roomvox-pot-php-files.txt';
    execSync(`find lib/ -name "*.php" -type f > ${phpFilesList}`, { cwd: ROOT });
    const phpPotPath = '/tmp/roomvox-pot-php.pot';
    execSync(
...
const header = `# RoomVox translation template.
